...
final class OnboardingPermissionController {
    ...
    func refreshPermissionStatuses() {
        ...
        if coordinator.stage == .permissions,
            OnboardingPermissionKind.required.allSatisfy({ diagnosedStatuses[$0]?.isGranted == true })
        {
            cancelRefreshTask()
    ...
    private func requestAccessibility() {
        ...
        // Let macOS register this signed app in the Accessibility database
        // before opening the list. Opening Settings immediately races TCC and
        // can show a list without Local Voice in it.
