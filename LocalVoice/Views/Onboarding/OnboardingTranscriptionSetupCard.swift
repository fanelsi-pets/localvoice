...
struct OnboardingTranscriptionSetupCard: View {
    ...
    private var apiKeyField: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            if let apiKeyInstructionsURL {
                Link(destination: apiKeyInstructionsURL) {
                    Label("How to create and copy the API key", systemImage: "questionmark.circle")
    ...
    private var apiKeyInstructionsURL: URL? {
        switch selectedProvider?.providerKey {
        case "OpenAI":
            return URL(string: "https://platform.openai.com/docs/quickstart#create-and-export-an-api-key")
        case "Gemini":
                ...
                if result.isValid {
                    guard APIKeyManager.shared.saveAPIKey(key, forProvider: providerKey) else {
                        ...
                        verificationMessage = String(
                            localized: "The key worked, but LocalVoice could not save it securely.")
                        verificationDetailMessage = nil
                        onVerificationChanged()
                        return
                    }
