...
struct SettingsView: View {
    ...
    var body: some View {
        Form {
            ...
            Section("Transcription") {
                Picker("Model", selection: transcriptionModelSelection) {
        ...
        .alert("Restart LocalVoice to Apply Language", isPresented: $showLanguageRestartAlert) {
            Button("OK", role: .cancel) {}
        } message: {
            Text("Your language change will take full effect after you quit and reopen LocalVoice.")
        }
    }
    ...
    private var transcriptionModelSelection: Binding<String> {
        Binding(
            ...
            set: { selectedName in
                guard let model = transcriptionModelManager.usableModels.first(where: { $0.name == selectedName })
                else { return }
                ...
                let updatedModes = ModeManager.shared.configurations.map { existing in
                    ...
                    if model.provider == .gemini { config.selectedLanguage = "auto" }
                    return config
                }
