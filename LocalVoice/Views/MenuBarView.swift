...
struct MenuBarView: View {
    ...
    private var completedOnboardingMenu: some View {
        Group {
            ...
            Menu {
                ForEach(transcriptionModelManager.usableModels, id: \.id) { model in
                    Button {
                        ...
                        let isSelected = transcriptionModelManager.currentTranscriptionModel?.name == model.name
                ...
                HStack {
                    ...
                    Text("Model: \(transcriptionModelManager.currentTranscriptionModel?.displayName ?? "None")")
                    Image(systemName: "chevron.up.chevron.down")
                        .font(.system(size: 10))
                }
            }
            ...
            Button("Gemini API Key") {
                showMainWindowAndNavigate(to: "Settings", reason: "Gemini API Key")
            }

            Button("Quit Local Voice") {
                NSApplication.shared.terminate(nil)
            }
        }
    }

    private func selectTranscriptionModel(_ model: any TranscriptionModel) {
        transcriptionModelManager.setDefaultTranscriptionModel(model)
        let updatedConfigurations = modeManager.configurations.map { existing in
            var config = existing
