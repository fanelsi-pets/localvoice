...
struct RecorderStatusDisplay: View {
    ...
    // Observed here, not by the caller, so this tiny view is the only thing that
    // redraws on every meter tick — not the whole recorder panel around it.
    @ObservedObject var meterPublisher: AudioMeterPublisher
    let menuBarHeight: CGFloat?

    init(currentState: RecordingState, meterPublisher: AudioMeterPublisher, menuBarHeight: CGFloat? = nil) {
    ...
    var body: some View {
        Group {
            if currentState == .enhancing {
                ...
                AudioVisualizer(audioMeter: meterPublisher.audioMeter, color: .white, isActive: true)
