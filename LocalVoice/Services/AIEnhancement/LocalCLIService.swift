...
enum LocalCLITemplate: String, CaseIterable, Identifiable {
    ...
    var commandTemplate: String {
        switch self {
        case .pi:
            return "pi -ne -ns -p --no-tools --system-prompt \"$LOCALVOICE_SYSTEM_PROMPT\" \"$LOCALVOICE_USER_PROMPT\""
        case .claude:
            return "claude -p \"$LOCALVOICE_FULL_PROMPT\""
        case .codex:
            return
                "TMPFILE=$(mktemp) && codex exec --skip-git-repo-check --output-last-message \"$TMPFILE\" \"$LOCALVOICE_FULL_PROMPT\" > /dev/null 2>&1 && cat \"$TMPFILE\" && rm \"$TMPFILE\""
        case .copilot:
            return "copilot -p \"$LOCALVOICE_FULL_PROMPT\" -s --no-ask-user --available-tools=__none__ 2>/dev/null"
        }
    }
}
    ...
    private func executeCommand(
        ...
        try await withCheckedThrowingContinuation { continuation in
            DispatchQueue.global(qos: .userInitiated).async {
                ...
                environment["LOCALVOICE_SYSTEM_PROMPT"] = systemPrompt
                environment["LOCALVOICE_USER_PROMPT"] = userPrompt
                environment["LOCALVOICE_FULL_PROMPT"] = fullPrompt
