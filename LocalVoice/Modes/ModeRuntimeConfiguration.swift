...
enum ModeRuntimeResolver {
    ...
    private static func resolvedModel(
        ...
        if let currentModel = transcriptionModelManager.currentTranscriptionModel,
            transcriptionModelManager.usableModels.contains(where: { $0.name == currentModel.name })
        {
            return currentModel
        }

        if let bundledWhisper = transcriptionModelManager.usableModels.first(where: { $0.name == "ggml-base" }) {
            return bundledWhisper
        }

        return transcriptionModelManager.usableModels.first(where: { $0.provider != .nativeApple })
    }
