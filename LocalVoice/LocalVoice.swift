...
struct LocalVoiceApp: App {
    ...
    @StateObject private var engine: LocalVoiceEngine
    ...
    init() {
            ...
            do {
                ...
                DispatchQueue.main.async {
                    ...
                    alert.informativeText = String(
                        localized:
                            "LocalVoice couldn't access its storage location. Your transcriptions will not be saved between sessions."
                    )
                ...
                fatalError(
                    "LocalVoice failed to initialize storage.\nPersistent:\n\(persistentDetail)\nIn-memory:\n\(memoryDetail)"
                )
            }
        }
        ...
        // Give a migrated installation one predictable recording shortcut.
        // This is a one-time seed only; if the user removes it later, their
        // choice remains respected on subsequent launches.
        let defaultShortcutSeedKey = "LocalVoiceDefaultRecordingShortcutSeeded"
        if !UserDefaults.standard.bool(forKey: defaultShortcutSeedKey) {
            ShortcutStore.seedShortcut(.rightCommand, for: .primaryRecording, replacingCleared: true)
            UserDefaults.standard.set(true, forKey: defaultShortcutSeedKey)
        }
    ...
    var body: some Scene {
        Window("Local Voice", id: AppWindowID.main) {
            Group {
